    ...
    while run:  
        ...
        if len(results.xywh[0])>0  and len(results.xywh[0])<=2:
            if len(results_y.xywh[0])>0  and len(results_y.xywh[0])<=1:
                ...
                if yawn_dconf.item()>0.85 and yawn_class.item()==0.0:
                    yawny+=1
                    if yawny>=3:
                        sound = st.empty()
                ...
                r_eye_dconf=results.xywh[0][1][4]

                l_eye_class=results.xywh[0][0][5]
                r_eye_class=results.xywh[0][1][5]

                if l_eye_class.item()==r_eye_class.item() and l_eye_class.item()==1.0:
                    if l_eye_dconf>0.85 and r_eye_dconf>0.85:
                        closy+=1
                        if closy>3:
                            sound = st.empty()
                            sound.markdown(html_string, unsafe_allow_html=True)  # will display a st.audio with the sound you specified in the "src" of the html_string and autoplay it
