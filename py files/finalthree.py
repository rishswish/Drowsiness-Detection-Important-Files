...
def infer_image_eye(img, size=None):
    model = torch.hub.load('ultralytics/yolov5', 'custom', path='last_eye_everyone.pt', force_reload=True)
...
def infer_image_yawn(img, size=None):
    model = torch.hub.load('ultralytics/yolov5', 'custom', path='last_yawn_everyone.pt', force_reload=True)
    model.conf = confidence
    result = model(img, size=size) if size else model(img)
    result.render()
    image = Image.fromarray(result.ims[0])
...
if img_or_vid=='Video':
    ...
    run = st.checkbox('Run')
    model_eye = torch.hub.load('ultralytics/yolov5', 'custom', path='last_eye_everyone.pt', force_reload=True)
    model_yawn = torch.hub.load('ultralytics/yolov5', 'custom', path='last_yawn_everyone.pt', force_reload=True)
    ...
    while run:  
        ...
        #Alarm for Yawn
        if len(results.xywh[0])>0  and len(results.xywh[0])<=2:
            if len(results_y.xywh[0])>0  and len(results_y.xywh[0])<=1:

                #for yawn
                yawn_dconf=results_y.xywh[0][0][4]
                yawn_class=results_y.xywh[0][0][5]            

                if yawn_dconf.item()>0.5 and yawn_class.item()==0.0:
                    yawny+=1
                    if yawny>=3:
                        sound = st.empty()
